...
const product = document.getElementById('products') as HTMLButtonElement;
const myFeedback = document.getElementById('myFeedback') as HTMLButtonElement;
...
async function filteredProductCat() {
    categoriesDiv.addEventListener("click", async (e) => {
        ...
        try {
            ...
            const data = await response.json();
            const filteredChoice = data.products.filter((pro: any) => {
                return pro.category === cate.toLowerCase().replace(' ', '-');
            });
            container.style.display = "grid";
            container.innerHTML = "";
            filteredChoice.forEach((product: any) => {
                const price = product.price.toFixed(2).split(".");
                container.innerHTML += `
                <div class="card" data-id="${product.id}">
                    <img src="${product.thumbnail}" />
...
container.addEventListener('click', (e) => {
    const target = e.target as HTMLElement;
    if (target.closest('#productDetails')) return;
...
async function showProductDetails(id: string | null) {
    if (!id) return;

    const response = await fetch(`https://dummyjson.com/products/${id}`);
    const product = await response.json();

    const details = document.getElementById("productDetails") as HTMLDivElement;
  ...
  `;
    ...
    const reviewList = document.getElementById('reviewsList') as HTMLDivElement;
    ...
    const close = document.querySelector('.close-btn') as HTMLButtonElement;
    close.addEventListener('click', () => details.classList.add("hidden"));

    const review = document.getElementById('reviewInput') as HTMLTextAreaElement;

    const send = document.getElementById('addReview') as HTMLButtonElement;
...
async function main() {
    const loginBtn = document.getElementById('toLogin') as HTMLButtonElement;
...
async function loadDashboardData() {
    ...
    const newList = document.getElementById("newList") as HTMLDivElement;
    newList.innerHTML = "";
    ...
    const mostList = document.getElementById("mostList") as HTMLDivElement;
  ...
  `;
    ...
    const topList = document.getElementById("topList") as HTMLDivElement;
...
function logIn() {
    ...
    login.addEventListener('click', () => {
        if (!name.value.trim()) {
            ...
            const writtenUser = document.getElementById('name') as HTMLBodyElement;
...
async function sendReview(productId: number, selectedRating: number) {
...
function addReviewToUl(newReview: any) {
    const reviewList = document.getElementById('reviewsList') as HTMLDivElement;
...
async function loadReview(productId: number, reviewList: any) {
...
async function loadMyFeedback() {
  ...
  `;
  const list = document.getElementById("myReviewsList") as HTMLDivElement;
  ...
  const productPromises = reviews.map((rev: any) =>
    fetch(`https://dummyjson.com/products/${rev.productId}`)
      .then(res => res.json())
  );
  ...
  reviews.forEach((rev: any, i: number) => {
...
function loadUser() {
    ...
    exit.disabled = false;
    const writtenUser = document.getElementById('name') as HTMLElement;
    const img = document.getElementById('img') as HTMLImageElement;
